#!/usr/bin/env python3
"""Non-measured Freqtrade re-evaluation under flexible completeness v2.1.
...
def field_contract_hash() -> str:
    payload = json.dumps(list(EXECUTION_FIELDS), separators=(",", ":"), ensure_ascii=True).encode("utf-8")
...
def explicit_source_fields(record: dict) -> tuple[dict[str, bool], list[str]]:
    ...
    fields = {
        "instrument_applicability": any(key in snippets for key in ("instrument_universe", "pair_scope", "universe")),
...
def evaluate(record: dict) -> dict:
    ...
    return {
        ...
        "execution_assumption_field_contract_sha256": field_contract_hash(),
        "execution_assumption_manifest": {"status": "not_present", "manifest_id": "execution_assumption_manifest_v1_1"},
...
def main() -> None:
    ...
    output = {
        "re_evaluation_version": "freqtrade_strategy_v2_1_flexible_completeness",
        ...
        "execution_assumption_contract": "execution_assumption_manifest_v1_1_required_for_missing_execution_fields",
        "execution_assumption_field_contract_sha256": field_contract_hash(),
