"""Validate Bybit daily historical L2 archives under the versioned v5 rule.

This file is intentionally separate from the original validator and from the
v2/v3/v4 validators. It preserves the v4 first-record partition rule and adds
one narrowly scoped terminal-boundary exception: a terminal duplicate boundary
record may overrun the exclusive UTC-day end by at most 2,001 ms. The
exception is valid only for the final record, only when it is the documented
terminal delta+snapshot duplicate, and only when every other integrity check
passes. It does not relax later-record timestamp checks, sequence checks, or
any non-terminal boundary.
...
boundary checks remain fail-closed.
"""
...
VALIDATION_RULE_VERSION = "bybit_l2_historical_integrity_v5"
BOUNDARY_GRACE_MS = 2_000
TERMINAL_BOUNDARY_EXCEPTION_MS = 2_001
PRE_START_BOUNDARY_GRACE_MS = 2_000
...
def validate(path: Path, symbol: str, day: str) -> dict:
    ...
    terminal_boundary_exceptions: list[dict] = []
            ...
            if not member_names:
                ...
                with archive.open(member_names[0], "r") as raw:
                    for line_no, line in enumerate(raw, 1):
                        ...
                        allowed_first_boundary = (
                            is_first_record
                            and typ == "snapshot"
                            and ts >= start_ms - PRE_START_BOUNDARY_GRACE_MS
                            and cts >= start_ms - PRE_START_BOUNDARY_GRACE_MS
                            and (ts < start_ms or cts < start_ms)
                        )
                        if ts < start_ms and not allowed_first_boundary:
                            errors.append(f"timestamp_before_day_ts:{line_no}:{ts}:{start_ms}")
                        if cts < start_ms and not allowed_first_boundary:
                            errors.append(f"timestamp_before_day_cts:{line_no}:{cts}:{start_ms}")
                        if allowed_first_boundary:
                            pre_start_events.append({
                                ...
                                "grace_ms": PRE_START_BOUNDARY_GRACE_MS,
                            })
                        ...
                        if overrun > BOUNDARY_GRACE_MS:
                            if (
                                overrun <= TERMINAL_BOUNDARY_EXCEPTION_MS
                                and terminal_dup
                                and line_no == records
                            ):
                                terminal_boundary_exceptions.append({
                                    "line_no": line_no,
                                    "overrun_ms": overrun,
                                    "type": typ,
                                    "u": u,
                                    ...
                                    "max_allowed_overrun_ms": TERMINAL_BOUNDARY_EXCEPTION_MS,
                                })
                            else:
    ...
    if records > 0 and first_ts is not None and first_cts is not None:
        first_is_snapshot_boundary_ok = (
            first_ts >= start_ms and first_cts >= start_ms
        ) or (
            first_ts >= start_ms - PRE_START_BOUNDARY_GRACE_MS
            and first_cts >= start_ms - PRE_START_BOUNDARY_GRACE_MS
        )
    ...
    result = {
        ...
        "pre_start_boundary_count": len(pre_start_events),
        "pre_start_boundaries": pre_start_events,
        "pre_start_boundary_grace_ms": PRE_START_BOUNDARY_GRACE_MS,
        "terminal_boundary_exception_max_ms": TERMINAL_BOUNDARY_EXCEPTION_MS,
        "terminal_boundary_exception_count": len(terminal_boundary_exceptions),
        "terminal_boundary_exceptions": terminal_boundary_exceptions,
        "errors": errors[:100],
    }
