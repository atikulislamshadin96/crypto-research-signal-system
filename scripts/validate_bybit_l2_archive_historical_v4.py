"""Validate Bybit daily historical L2 archives under the versioned v4 rule.

This file is intentionally separate from the original validator and from the
v2/v3 validators. It adds one narrowly scoped archive-boundary exception: the
first record may be a snapshot whose `ts` and/or matching-engine timestamp
(`cts`) is slightly before the UTC day start. This reflects Bybit's documented
distinction between `cts` and `ts` and is allowed only when both timestamps are
within PRE_START_BOUNDARY_GRACE_MS, only on the first record, and only when
every later record is in-window.
...
VALIDATION_RULE_VERSION = "bybit_l2_historical_integrity_v4"
